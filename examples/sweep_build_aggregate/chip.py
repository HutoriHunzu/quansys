...
from drawing.transmon import draw_transmon_with_antenna
from drawing.export_to_pyaedt.parser import parse_component, ExportConfig, parse_component_multi
...
def build(hfss: Hfss,
    ...
    transmon_parameters = load_relevant_parameters(kwargs, TransmonParameters, with_prefix='transmon_')
    ...
    lst_points, independent_variables, dependent_variables = parse_component_multi(transmon_gds, export_config)
    ...
    junction = modeler.create_rectangle(origin=[f'transmon_junction_left_arm_x - {VarsNames.junction_width} / 2',
                                        ...
                                        sizes=['transmon_junction_right_arm_z - transmon_junction_left_arm_z',
    ...
    junction_mesh_box = modeler.create_box(
        ...
        sizes=[f'{VarsNames.junction_width} * {VarsNames.junction_mesh_box_size_scale}',
    ...
    junction_line_start_point = ['transmon_junction_left_arm_x', 'transmon_junction_left_arm_y',
                                 'transmon_junction_left_arm_z']
    junction_line_end_point = ['transmon_junction_right_arm_x', 'transmon_junction_right_arm_y',
                               'transmon_junction_right_arm_z']
    junction_line = modeler.create_polyline([junction_line_start_point, junction_line_end_point],
                                            name='junction_line')

    # adding lump element to junction using the same start and end point of the line.
    # for that we need the evaluated points
    junction_start_point_evaluated = 0, junction.bounding_box[1], junction.bounding_box[2]
    junction_end_point_evaluated = 0, junction.bounding_box[4], junction.bounding_box[5]

    lumped = hfss.assign_lumped_rlc_to_sheet('transmon_junction',
                                             start_direction=[junction_start_point_evaluated,
                                                              junction_end_point_evaluated],
                                             inductance=10,
    ...
    hfss['readout_e1_z'] = f'transmon_orientation_port_z - {VarsNames.transmon_readout_gap} - readout_size_z'
