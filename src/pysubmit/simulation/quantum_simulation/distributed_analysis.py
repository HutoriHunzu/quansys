...
class DistributedAnalysis:
    def __init__(self, hfss: Hfss, modes_to_labels: dict, junctions_infos: List[ConfigJunction]):
        self.hfss = hfss
    ...
    def set_mode(self, mode):
        mode = str(mode)
        one_hot_mode_dict = {f"{i}": ("0", "0deg") for i in range(1, self.number_of_modes + 1)}
        one_hot_mode_dict[mode] = ("1", "0deg")

        # edit sources such that it is only one active
        self.hfss.edit_sources(assignment=one_hot_mode_dict, eigenmode_stored_energy=False)

    def _calculate_line_voltage(self, freq, line_object_name, line_inductance, use_smooth=False):
    ...
    def parse_modes_and_set_number_of_modes(self):
        ...
        modes_names = self.post_api.available_report_quantities(quantities_category='Eigen Modes')
        ...
        for mode_number in self.modes_to_labels.keys():
            ...
            if f'Mode({mode_number})' not in modes_names:
                raise ValueError(f'Cannot find mode number: {mode_number} in the mode solutions: {modes_names}')
    ...
    # pass
    def calculate_peak_current_and_voltage(self, mode_frequency, junction_infos: Tuple[ParsedJunctionValues, ...]):

        def helper():
            for info in junction_infos:
                yield self._calculate_line_voltage(mode_frequency, info.info.name, info.inductance.value)

        peak_currents, peak_voltages = list(zip(*helper()))

        return np.array(peak_currents), np.array(peak_voltages)

    def calculate_inductance_and_capacitance_energies(self,
    ...
    def calculate_p_junction(self,
        ...
        peak_currents, peak_voltages = self.calculate_peak_current_and_voltage(mode_frequency, junctions_infos)

        # calculate inductance and capacitance energies
        inductance_energy, capacitance_energy = self.calculate_inductance_and_capacitance_energies(
            peak_currents,
            peak_voltages,
            junctions_infos
        )
    ...
    def get_all_frequencies_and_q_factors_with_labels(self):
        d = {label: self.get_mode_frequency_and_q_factor(mode_number)
    ...
    def _parse_junctions_infos_to_values(self, label_to_freq_and_qfactor) -> Tuple[ParsedJunctionValues, ...]:
    ...
    def main(self) -> ParticipationDataset:
        ...
        label_to_frequency_and_q_factor = self.get_all_frequencies_and_q_factors_with_labels()

        # parsing junctions infos into parsed junction values
        parsed_junction_infos = self._parse_junctions_infos_to_values(label_to_frequency_and_q_factor)
        ...
        for mode_number, label in self.modes_to_labels.items():
            ...
            result[label] = self.calculate_p_junction(frequency,
                                                      two_times_total_peak_magnetic_energy,
