...
class FrequencyAndQualityFactorFormatter(BaseFormatter):
    ...
    def _get_all_modes_to_freq_and_quality_factor(self, setup) -> dict[int, dict[str, float]]:
        # # post_api = hfss.post
        #
        ...
        return dict(map(lambda x: (x, self._get_mode_to_freq_and_quality_factor(x)),
                        range(1, number_of_modes + 1)))

    def _get_mode_to_freq_and_quality_factor(self, mode_number: int):
        freq_sol = self.setup.get_solution_data(expressions=f'Mode({mode_number})')
        q_sol = self.setup.get_solution_data(expressions=f'Q({mode_number})')
