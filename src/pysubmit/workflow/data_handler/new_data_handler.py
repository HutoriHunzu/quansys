...
class Metadata(BaseModel):
    ...
    def load(cls, base_path: Path) -> Self:
        ...
        """
        return json_read(base_path / 'metadata.json', cls=cls)
...
class DataHandler(BaseModel):
    ...
    def register_identifier(self, identifier: str) -> Path:
        ...
        metadata = Metadata.load(self.last_iteration_path)
    ...
    def summarize_iterations(self) -> dict[str, Any]:
        """
        ...
        if not self.iterations_directory.exists():
            return summary

        for iteration_folder in self.iterations_directory.iterdir():
            if iteration_folder.is_dir():
                metadata_path = iteration_folder / "metadata.json"
                if metadata_path.exists():
                    raw_metadata = json.loads(metadata_path.read_text())
                    # Reconstruct Metadata with the correct base_path.
                    metadata = Metadata(
                        iteration_number=raw_metadata.get("iteration_number", -1),
                        base_path=iteration_folder,
                        id_to_description=raw_metadata.get("id_to_description", {})
                    )
                    summary[iteration_folder.name] = metadata.model_dump()
        return summary
    ...
    def _iter_metadata(self) -> list[Metadata]:
        """
        ...
        """
        metadata_files = list(self.iterations_directory.rglob("metadata.json"))
        metas: list[Metadata] = []
        for mfile in metadata_files:
            try:
                meta = Metadata.load(mfile.parent)
                metas.append(meta)
            except Exception:
                continue
        metas.sort(key=lambda m: m.iteration_number)
        return metas

    def aggregate_by_config(self, config: dict[str, tuple[str, ...]], adapter: Any = None) -> dict[
        ...
        for meta in self._iter_metadata():
            for agg_name, id_tuple in config.items():
                agg_row: dict[str, Any] = {"iteration_number": meta.iteration_number}
                for identifier in id_tuple:
                    if identifier in meta.id_to_description:
                        rel_path = meta.id_to_description[identifier].path
                        data_file_path = meta.base_path / rel_path
                        if data_file_path.exists():
                            data = json.loads(data_file_path.read_text())
                            result_obj: BaseSimulationOutput = adapter.validate_python(data)
                            flat = result_obj.flatten()
                            # Merge flattened dict directly.
                            agg_row.update(flat)
                aggregated_by_group[agg_name].append(agg_row)
        return aggregated_by_group

    def save_aggregation_to_csv(self, config: dict[str, tuple[str, ...]], adapter: Any = None) -> None:
        """
        ...
        aggregated = self.aggregate_by_config(config, adapter)
        for group, rows in aggregated.items():
            df = pd.DataFrame(rows)
            csv_file = self.aggregations_directory / f"{group}.csv"
            df.to_csv(csv_file, index=False)

    # def save_aggregation_to_csv(self, config: dict[str, tuple[str, ...]], csv_path: Path, adapter: Any = None) -> None:
    #     """
    #     Stacks all aggregated rows into a Pandas DataFrame and saves the result as a CSV file.
    #
    #     Args:
    #         config (dict[str, tuple[str, ...]]): Aggregation configuration mapping aggregation names to tuples of identifiers.
    #         csv_path (Path): The file path to save the CSV.
    #         adapter: Optional data adapter with a .validate_python method. Defaults to ANALYSIS_ADAPTER.
    #     """
    #     import pandas as pd
    #
    #     rows = self.aggregate_by_config(config, adapter)
    #     df = pd.DataFrame(rows)
