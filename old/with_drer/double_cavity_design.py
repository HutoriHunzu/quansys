...
class Ellipse:
    ...
    def get_coordinates(self):
        thetas = np.linspace(self.start_angle, self.stop_angle, self.num_points)
...
class Line:
    ...
    def get_coordinates(self):
        xs = np.linspace(self.start_point[0], self.end_point[0], self.num_points)
        ys = np.linspace(self.start_point[1], self.end_point[1], self.num_points)
        return xs, ys
...
def half_cell_length(
    Rc: float, Lc: float, a: float, b: float, RI: float, decimal_precision: int = 6
):
    ...
    def coupler_parameter_equation(phi):
        ...
        return (
            Rc * np.sin(np.arctan(np.tan(phi) * Rc * b / (Lc * a)))
            - a * np.sin(phi)
            - RI
            - a
        )
    ...
    return (
        np.round(length, decimal_precision),
        np.round(cavity_parameter, decimal_precision),
        np.round(coupler_parameter, decimal_precision),
    )
...
def fillet_circle_location(Rc: float, Lc: float, r_fillet: float):
    ...
    for parameter in (fillet_circle_parameter, cavity_parameter):
        assert parameter < (np.pi / 2) or parameter > 0, f"parameter out of range"
...
class TwoModeCavityCurveParameters(TwoModeCavityParameters):
    ...
    def __post_init__(self):
        ...
        """
        (
            self.half_cell_fillet_circle_y0,
            self.half_cell_parameter_start,
            self.half_cell_fillet_circle_parameter,
        ) = fillet_circle_location(
            self.half_cell_Rc, self.half_cell_Lc, self.half_cell_fillet_radius
        )
        (
            self.half_cell_length,
            self.half_cell_parameter_stop,
            self.coupler_parameter_start,
        ) = half_cell_length(
            self.half_cell_Rc,
            self.half_cell_Lc,
            self.coupler_a,
        ...
        (
            self.full_cell_coupler_side_length,
            self.full_cell_coupler_side_parameter,
            coupler_parameter_stop,
        ) = half_cell_length(
            self.full_cell_Rc,
            self.full_cell_Lc,
            self.coupler_a,
        ...
        self.coupler_parameter_stop = np.round(
            3 * np.pi - coupler_parameter_stop, self.decimal_precision
        )  # adjusting for proper quarter

        (
            self.full_cell_wg_side_length,
            self.full_cell_wg_side_parameter,
            self.cavity_drive_wg_parameter,
        ) = half_cell_length(
            self.full_cell_Rc,
            self.full_cell_Lc,
            self.coupler_a,
...
class Cavity(ABC):
    ...
    @staticmethod
    def _filter_close_coordinates(coordinates, tolerance=1.0e-6):
    ...
    def build_hfss_model(self, hfss: Hfss):
        ...
        points = [
            [0, self.y_coordinates[i], self.x_coordinates[i]]
            for i in range(len(self.x_coordinates))
        ]  # Choosing Z as cavity axis

        """ MODEL"""

        # CAVITY
        # , xsection_bend_type='curved', cover_surface=True)
        cavity = modeler.create_polyline(
            points, name="cavity", segment_type="Spline", material="vacuum"
        )
    ...
    def plot(self, plot_double_side=True):
        ...
        if plot_double_side:
            ax.plot(
                self.x_coordinates, [-y for y in self.y_coordinates], color="tab:blue"
            )
            ax.set_ylim(
                [-1.1 * np.max(self.y_coordinates), 1.1 * np.max(self.y_coordinates)]
            )
...
class TwoModeCavity(Cavity):
    ...
    def parameters(self, *args, **kwargs):
        raise NotImplementedError(
            "Direct modification of parameters is not allowed. Please create new cavity instance"
        )

    def _define_structure(self):
        transmon_wg_coupler = Ellipse(
            "transmon_wg_coupler",
        ...
        connecting_line = Line(
            ...
            start_point=[
                0,
                self.curve_parameters.transmon_wg_radius
                + self.curve_parameters.transmon_coupler_a,
            ],
            end_point=[0, self.curve_parameters.half_cell_fillet_circle_y0],
            num_points=self.curve_parameters.points_per_segment // 4,
        )

        fillet_circle = Ellipse(
            "fillet",
        ...
        half_cell = Ellipse(
            "half_cell",
            self.curve_parameters.half_cell_Lc,
            0,
            self.curve_parameters.half_cell_Rc,
            0,
            self.curve_parameters.half_cell_parameter_start,
            self.curve_parameters.half_cell_parameter_stop,
        ...
        coupler = Ellipse(
            ...
            self.curve_parameters.Iris_radius + self.curve_parameters.coupler_a,
        ...
        cavity_drive_wg_coupler = Ellipse(
            ...
            + self.curve_parameters.coupler_a,
            self.curve_parameters.cavity_drive_wg_parameter,
            3 * np.pi / 2,
            self.curve_parameters.points_per_segment // 2,
        )
        cavity_drive_wg = Line(
            "cavity drive waveguide",
            start_point=[
                self.curve_parameters.half_cell_length
                + self.curve_parameters.full_cell_coupler_side_length
                + self.curve_parameters.full_cell_wg_side_length,
            ...
            end_point=[
                ...
                + self.curve_parameters.full_cell_coupler_side_length
                + self.curve_parameters.full_cell_wg_side_length
                + self.curve_parameters.cavity_drive_wg_length,
                self.curve_parameters.cavity_drive_wg_radius,
            ],
            num_points=self.curve_parameters.points_per_segment // 2,
        )
